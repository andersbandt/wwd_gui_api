...
class tabAFE:
    def __init__(self, master, basefilepath):
        ...
        self.data_folder = "/data/afe_data/"
    ...
    def init_fr_analysis(self):
        ...
        def refresh_file_list():
            file_list = get_file_list(self.basefilepath + "\data")
            file_dropdown = tk.OptionMenu(self.fr_analysis, selected_file, *file_list)
            file_dropdown.grid(row=0, column=0, pady=10)
    ...
    def start_record(self):
        ...
        error_flag |= self.ser_obj.init_data_process(self.basefilepath,
                                                     ["ID", "fifo", "data", "timestamp"],
                                                     filename_ext=file_str_ext)
    ...
    def analyze_file(self, filename):
        ...
        afe_data = processor.load_csv(self.basefilepath + self.data_folder + filename)
        if afe_data is None:
    ...
    def graph_file(self, filename):
        ...
        if afe_data is None:
            gui_helper.alert_user("Something wrong with data!",
                                  ...
                                  kind="error")
            return False
        afe_analysis.graph_afe(afe_data)

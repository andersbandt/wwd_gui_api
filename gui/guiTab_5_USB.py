...
# TODO: possibly remove the threading from all these action functions... I think the only one that needs it is the print
#   and this can be handled in one of the called functions
    ...
    def init_fr_serial(self):
        ...
        btn_connect_serial.grid(row=2, column=1, padx=15, pady=8)
        btn_disconnect_serial = Button(self.fr_serial, text="Disconnect COM",
                                       command=lambda: threading.Thread(target=self.serial_close).start(),
                                       bg="orange", fg="black", height=2, width=15)
        btn_disconnect_serial.grid(row=3, column=1, padx=15, pady=8)
        ...
        while status:
            try:
                if self.ser_obj.serObj.inWaiting() > 0:  # Wait here until there is data
                    data = self.ser_obj.get_data("timestamp")
                    data = data[1:-1] # remove the "\n" I add for timestamp mode
                    if data is not None:
                        self.prompt2.print(data)
            except serial.serialutil.SerialException as e:
                print(e)
                status = False
            except UnicodeDecodeError as e:
                self.prompt1.print(e.__repr__())
        self.ser_status = False
        my_oval = self.canvas1.create_oval(50 * .25, 50 * .25, 50 * .75, 50 * 0.75)  # x0, y0, x1, y1
        self.canvas1.itemconfig(my_oval, fill="red")  # Fill the circle with RED
        return False
    ...
    def serial_init(self, serial_port):
        ...
        try:
            self.ser_obj = SerialReader(serial_port, 115200, log=True)
        except serial.serialutil.SerialException as e:
